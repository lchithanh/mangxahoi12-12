...
<div class="container py-4">
    ...
    <!-- Header nhà hàng -->
    <div class="header mb-4" style="background: linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url('{{ $nhaHang->anh_dai_dien ? asset('storage/'.$nhaHang->anh_dai_dien) : 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4' }}') center/cover no-repeat; border-radius:10px; padding:50px 20px; color:#fff; text-align:center;">
            ...
            <div class="card text-center mb-4">
                <div class="card-body">
                    @if(!empty($nhaHang->anh_dai_dien))
                        <img src="{{ asset('storage/' . $nhaHang->anh_dai_dien) }}" class="rounded-circle mb-3" alt="{{ $nhaHang->ten_nha_hang }}" style="width:150px; height:150px; object-fit:cover;">
                    @else
...
@if(session('user'))
    <div class="d-flex gap-2 mt-3">
        ...
        @if($nhaHang->chuSoHuu)
            <a href="{{ route('nhantin.show', $nhaHang->chuSoHuu->ma_nguoi_dung) }}" class="btn btn-success btn-sm flex-fill">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        ...
                        @foreach($nhaHang->menu as $m)
                            <div class="menu-item d-flex gap-3 mb-3">
                                <img src="{{ asset('storage/'.$m->anh) }}" alt="{{ $m->ten_mon }}" style="width:100px;height:100px; object-fit:cover; border-radius:8px;">
                                <div>
                                    <h6>{{ $m->ten_mon }}</h6>
                                    <p>{{ $m->mo_ta }}</p>
