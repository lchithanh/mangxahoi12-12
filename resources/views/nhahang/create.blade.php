...
<div class="container mt-5">
    ...
    <form action="{{ route('nhahang.store') }}" method="POST" enctype="multipart/form-data">
        ...
        {{-- Chủ sở hữu --}}
        <div class="mb-3">
            <label for="chu_so_huu_ten" class="form-label">Tên Chủ Sở Hữu</label>
            <input type="text" name="chu_so_huu_ten" id="chu_so_huu_ten" class="form-control" placeholder="Nhập tên chủ sở hữu" value="{{ old('chu_so_huu_ten') }}" required>
        </div>

        {{-- Tên nhà hàng --}}
        <div class="mb-3">
            <label for="ten_nha_hang" class="form-label">Tên Nhà Hàng</label>
            <input type="text" name="ten_nha_hang" id="ten_nha_hang" class="form-control" value="{{ old('ten_nha_hang') }}" required>
        </div>

        {{-- Địa chỉ --}}
        <div class="mb-3">
            <label for="dia_chi" class="form-label">Địa Chỉ</label>
            <input type="text" name="dia_chi" id="dia_chi" class="form-control" value="{{ old('dia_chi') }}" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="ma_khu_vuc" id="ma_khu_vuc" class="form-select" onchange="toggleKhuVucKhac()">
                ...
                @foreach($khuVucs as $khuVuc)
                    <option value="{{ $khuVuc->id }}" {{ old('ma_khu_vuc') == $khuVuc->id ? 'selected' : '' }}>
                        {{ $khuVuc->ten_khu_vuc }}
                    </option>
                @endforeach
                <option value="khac" {{ old('ma_khu_vuc') == 'khac' ? 'selected' : '' }}>Khác</option>
            </select>
            <input type="text" name="ten_khu_vuc_moi" id="ten_khu_vuc_moi" class="form-control mt-2" placeholder="Nhập khu vực mới" style="display:none;" value="{{ old('ten_khu_vuc_moi') }}">
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="phan_loai" id="phan_loai" class="form-select" onchange="togglePhanLoaiKhac()">
                ...
                @foreach($phanLoais as $phanLoai)
                    <option value="{{ $phanLoai->ten_phan_loai }}" {{ old('phan_loai') == $phanLoai->ten_phan_loai ? 'selected' : '' }}>
                        {{ $phanLoai->ten_phan_loai }}
                    </option>
                @endforeach
                <option value="khac" {{ old('phan_loai') == 'khac' ? 'selected' : '' }}>Khác</option>
            </select>
            <input type="text" name="phan_loai_moi" id="phan_loai_moi" class="form-control mt-2" placeholder="Nhập phân loại mới" style="display:none;" value="{{ old('phan_loai_moi') }}">
        </div>
        ...
        <a href="{{ route('baiviet.index') }}" class="btn btn-secondary">Hủy</a>
    </form>
</div>
